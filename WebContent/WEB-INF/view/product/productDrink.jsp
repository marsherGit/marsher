...
<body>
	<div>
		<form class="form-horizontal" method="post">
			<ul id="drinkList" class="col-md-12">
				<c:if test="${count == 0}">
      ...
      관련 제품이 없습니다.
			...
			<c:if test="${count > 0}">
				<c:forEach var="drinkList" items="${drinkList}">
					...
					<ul class="col-md-3" value="${drinkList.pro_name}"
						style="text-align: center;">

						<table id="product" style="margin: auto; text-align: center;"
							border="1px" bordercolor="6D98B4" onClick="#" data-toggle="modal"
							data-target="#${drinkList.pro_num}">
							<tr>
								<td><img src="/Marsher/images/${drinkList.pro_image}"
									style="width: 200px; height: 210px;" /><br /> <strong>${drinkList.pro_name}</strong><br />
									(${drinkList.pro_container} - ${drinkList.pro_volume}ml)<br />
									${drinkList.pro_calorie} kcal</td>
							</tr>
...
<!-- pagenation -->
  ...
  <c:if test="${ endPage > pageCount }">
	...
	<div class="row pagingArea text-center">
		<ul class="pagination pagination-sm">
			<c:if test="${ startPage > pageBlock }">
				<li class="prev"><a href="<c:url value="/product/productDrink.do?pageNum=${ startPage - pageBlock }" />">&lt;&lt;</a></li>
			</c:if>
			<c:forEach var="i" begin="${ startPage }" end="${ endPage }">
		  	<li><a href="<c:url value="/product/productDrink.do?pageNum=${ i }" />" class="page${ i }">${ i }</a></li>
		  </c:forEach>
		  <c:if test="${ endPage < pageCount }">
		  	<li class="next"><a href="<c:url value="/product/productDrink.do?pageNum=${ startPage + pageBlock }" />">&gt;&gt;</a></li>
		  </c:if>
		</ul>
	</div>
	</c:if>
	...
	<c:forEach var="drinkList" items="${drinkList}">
		<!-- Modal -->
		<div id="${drinkList.pro_num}" class="modal fade" role="dialog">
			<div class="modal-dialog modal-lg">
				...
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">주류 - 제품 소개</h4>
					</div>
									...
									<div class="row">
										<div class="col-sm-4"
											...
											<label for="pro_image">제품 사진</label> <img style='cursor: hand'
												src="/Marsher/images/${drinkList.pro_image}"
										...
										<div class="col-sm-4" style='width: 500px; padding: 35px'>
											<table class="table table-bordered">
												<tr>
													...
													<td>${drinkList.pro_num}</td>
												</tr>
												<tr>
													<td class="text-center"><strong>상품명</strong></td>
													<td>${drinkList.pro_name}</td>
												</tr>
												<tr>
													<td class="text-center"><strong>용량</strong></td>
													<td>${drinkList.pro_volume}ml</td>
												</tr>
												<tr>
													<td class="text-center"><strong>패키지타입</strong></td>
													<td>${drinkList.pro_container}</td>
												</tr>
												<tr>
													<td class="text-center"><strong>칼로리</strong></td>
													<td>${drinkList.pro_calorie}kcal</td>
												</tr>
											</table>
											<div class="row text-right">
											<c:if test="${ memId eq 'admin' }"> 	
											<a href="<c:url value="/product/drinkDeletePro.do?pro_num=${drinkList.pro_num}"/>" class="btn btn-danger btn-sm" onclick="return deleteCheck();">제품삭제</a>								
											</c:if>
											</div>
										</div>
