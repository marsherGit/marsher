...
<body>
	<div>
		<form class="form-horizontal" method="post">
			<ul id="sodaList" class="col-md-12">
				<c:if test="${count == 0}">
<table width="700" border="1" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center">
      관련 제품이 없습니다.
			...
			<c:if test="${count > 0}">
				<c:forEach var="sodaList" items="${sodaList}">

					<ul class="col-md-3" value="${sodaList.pro_name}"
  ...
  <c:if test="${ endPage > pageCount }">
	...
	<div class="row pagingArea text-center">
		<ul class="pagination pagination-sm">
			<c:if test="${ startPage > pageBlock }">
				<li class="prev"><a href="<c:url value="/product/productSoda.do?pageNum=${ startPage - pageBlock }" />">&lt;&lt;</a></li>
			</c:if>
			<c:forEach var="i" begin="${ startPage }" end="${ endPage }">
		  	<li><a href="<c:url value="/product/productSoda.do?pageNum=${ i }" />" class="page${ i }">${ i }</a></li>
		  </c:forEach>
		  <c:if test="${ endPage < pageCount }">
		  	<li class="next"><a href="<c:url value="/product/productSoda.do?pageNum=${ startPage + pageBlock }" />">&gt;&gt;</a></li>
		  </c:if>
		</ul>
	</div>
	</c:if>
	...
	<c:forEach var="sodaList" items="${sodaList}">
		<!-- Modal -->
		<div id="${sodaList.pro_num}" class="modal fade" role="dialog">
			<div class="modal-dialog modal-lg">
				...
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">탄산 - 제품 소개</h4>
					</div>
									...
									<div class="row">
										<div class="col-sm-4"
											...
											<label for="pro_image">제품 사진</label> <img style='cursor: hand'
												src="/Marsher/images/${sodaList.pro_image}"
										...
										<div class="col-sm-4" style='width: 500px; padding: 35px'>
											<table class="table table-bordered">
												<tr>
													...
													<td>${sodaList.pro_num}</td>
												</tr>
												<tr>
													<td class="text-center"><strong>상품명</strong></td>
													<td>${sodaList.pro_name}</td>
												</tr>
												<tr>
													<td class="text-center"><strong>용량</strong></td>
													<td>${sodaList.pro_volume}ml</td>
												</tr>
												<tr>
													<td class="text-center"><strong>패키지타입</strong></td>
													<td>${sodaList.pro_container}</td>
												</tr>
												<tr>
													<td class="text-center"><strong>칼로리</strong></td>
													<td>${sodaList.pro_calorie}kcal</td>
												</tr>
											</table>
											<div class="row text-right">
											<c:if test="${ memId eq 'admin' }"> 	
											<a href="<c:url value="/product/sodaDeletePro.do?pro_num=${sodaList.pro_num}"/>" class="btn btn-danger btn-sm" onclick="return deleteCheck();">제품삭제</a>								
											</c:if>
											</div>
										</div>
