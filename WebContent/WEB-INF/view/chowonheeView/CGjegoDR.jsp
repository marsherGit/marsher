    ...
    $('img').click(function(){
       ...
       $(name).change(function(){
     	...
     	/* 	  alert(data1); */
     		 ...
     		 var url="<%=cp%>/chowonheeView/ajax/CGjegoDR";
     		 var pro_name ="pro_name="+data;  
 	...
 	/* 	  alert(data1); */
  	...
  	$(".jegoDRtable1 #pro_count").change(function(){
  		...
  		var url="<%=cp%>/chowonheeView/ajax/CGjegoDR1";
...
<script>
	function check(){
  		for(var i=1;i<=$("#table tr:eq(0)").next().attr("class").replace("jegoDRtable","");i++){
  			...
  			if($("#select2 option:selected").val() == "선택"){
  				alert("입고장소를 선택하지 않았습니다.");
  				$("#select2").focus();
  				return false;
  			}
...
<body>
<br/>
<form method="post" name="jegoDR" action="CGjegoDRPro" onsubmit="return check()">
<table id="table" name="table" border="1" cellpadding="0" cellspacing="0" align="center" border="1" class="table-striped">
  ...
  <input type="text"  style="text-align:center;" id="number" class="btn btn-default" name="number" value="1" readonly/>
	...
	<td align="center"  width="100" name="pro_container"><input type="text" style="text-align:center; width:100px;" id="pro_container" class="btn btn-default" name="pro_container" value="${product.pro_container}" readonly/></td>
    <td align="center"  width="100" name="pro_volume" ><input type="text" style="text-align:center; width:100px;" id="pro_volume" class="btn btn-default" name="pro_volume" value="${product.pro_volume}" readonly/></td>
    <td align="center" width="100" name="pro_count"><input type="number" style="text-align:right; width:100px;" id="pro_count" class="btn btn-default" name="pro_count" value="0" min="0"/></td>
    <td align="center"  width="50" />
  </tr>
  ...
  <td  width="150" align="center" colspan="2">입고장소</td>
        <td align="center" width="400" name="inout_receiver" colspan="4">
  	    <select id="select2" style="width:400px;" name="st_name" class="form-control input-sm">
		<option>선택</option>
		<c:forEach var="store" items="${storeList}" varStatus="status">
		<option value="${store.st_name}">${store.st_name}</option>
		</c:forEach>
		</select> 
   	</td>
  <td align="center"  width="50" ></td>  
  ...
  <td colspan="6"><textarea id="comment" class="btn btn-default" name="comment" rows="6" cols="100" value="없음" onfocus="this.value=''" onfocusout="checkNull2()">없음</textarea></td>
  </tr>
...
<input type="reset" value="취소하기" class="btn btn-default" onclick="document.location.href='CGjegoDR'">   
<input type="button" value="뒤로가기" class="btn btn-default" onclick="document.location.href='changgoCGList'">
</center>  
</form>
</body>
</html>